...
public class DirectedAcyclicGraph<V, E> extends SimpleDirectedGraph<V, E> {
	 ...
	 *             If either fromVertex or toVertex is not a member of the graph
	 * @throws CycleFoundException
	 *             if the edge would induce a cycle in the graph
	 * 
	...
	private void dfsF(V vertex, Set<V> df, Visited visited,
		...
		for (E outEdge : outgoingEdgesOf(vertex)) {
			...
			Integer nextVertexTopoIndex = topoOrderMap
	...
	private void dfsB(V vertex, Set<V> db, Visited visited,
		...
		for (E inEdge : incomingEdgesOf(vertex)) {
			...
			Integer previousVertexTopoIndex = topoOrderMap
	...
	private class TopoIterator implements Iterator<V> {
		...
		public void remove() {
			...
			if (null != (vertexToRemove = topoOrderMap
					.getVertex(currentTopoIndex))) {
