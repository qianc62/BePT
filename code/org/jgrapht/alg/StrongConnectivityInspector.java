...
public class StrongConnectivityInspector<V, E> {
	...
	public List<Set<V>> stronglyConnectedSets() {
		if (stronglyConnectedSets == null) {
			...
			DirectedGraph<V, E> inverseGraph = new EdgeReversedGraph<V, E>(
					graph);
	...
	private void createVertexData() {
		...
		for (V vertex : graph.vertexSet()) {
			vertexToVertexData.put(vertex, new VertexData2<V>(vertex, false,
					false));
		}
	}
	...
	private static final class VertexData1<V> extends VertexData<V> {
		...
		private VertexData1(VertexData<V> finishedData, boolean discovered,
				boolean finished) {
