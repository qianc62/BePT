...
public class FloatDistanceMatrix extends DistanceMatrix {
	...
	/*
	 * public void add(DistanceMatrix other) { if(itemCount != other.itemSize())
	 * { throw new
	 * AssertionError("Cannot add two distance matrices of different sizes!"); }
	 * else { for(int x=0; x<itemCount; x++) { for(int y=0; y<=x; y++) { double
	 * value = get(x, y) + other.get(x, y); set(x, y, value); if(value > max) {
	 * max = (float)value; } if(value < min) { min = (float)value; } } } } }
	 */
