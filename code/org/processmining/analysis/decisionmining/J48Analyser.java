...
public class J48Analyser extends DecisionAnalyser {
	...
	public J48Analyser() {
		...
		unprunedTree = new GUIPropertyBoolean("Use unpruned tree",
				((J48) myClassifier).unprunedTipText(), ((J48) myClassifier)
						.getUnpruned());
		confidence = new GUIPropertyFloat("Confidence treshold for pruning",
				((J48) myClassifier).confidenceFactorTipText(),
				((J48) myClassifier).getConfidenceFactor(), (float) 0.0,
				(float) 1.0, (float) 0.01);
		minNoInstances = new GUIPropertyInteger(
				"Minimun number of instances in any leaf", ((J48) myClassifier)
						.minNumObjTipText(), ((J48) myClassifier)
						.getMinNumObj(), 0, 1000);
		...
		numberFolds = new GUIPropertyInteger(
				...
				((J48) myClassifier).numFoldsTipText(), ((J48) myClassifier)
						.getNumFolds(), 1, 100);
		...
		retainInstanceInfo = new GUIPropertyBoolean(
				"Retain instance information", ((J48) myClassifier)
						.saveInstanceDataTipText(), ((J48) myClassifier)
						.getSaveInstanceData());
		smoothing = new GUIPropertyBoolean(
				"Smooth the probability estimates using Laplace smoothing",
				((J48) myClassifier).useLaplaceTipText(), ((J48) myClassifier)
						.getUseLaplace());
		seed = new GUIPropertyInteger("Seed for shuffling data",
				((J48) myClassifier).seedTipText(), ((J48) myClassifier)
						.getSeed(), 0, 100);
	}
	...
	protected JPanel createResultVisualization() {
		...
		try {
			...
			((J48ResultPanel) resultViewPanel)
					.setTreeVisualizer(new TreeVisualizer(null,
